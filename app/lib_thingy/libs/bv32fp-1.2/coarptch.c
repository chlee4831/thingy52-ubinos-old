...
Float *xw, Float *xwdm, Float *dfm, /* (i/o) ellipse low pass filter memory */
int cpplast) /* in the undecimated domain Q3 */
{
    ...
    {
        if ((cor2[n] * energy[n - 1] > cor2[n - 1] * energy[n]) && (cor2[n] * energy[n + 1] > cor2[n + 1] * energy[n]) && (cor2[n] > 0))
        {
            idx[npeaks] = n;
            npeaks++;
        }
    ...
    for (i = 0; i < npeaks; i++)
    {
        ...
        if (cor2[n + 1] * energy[n - 1] > cor2[n - 1] * energy[n + 1])
        { /* if right side */
    ...
    energym = 1.0F;
    for (i = 0; i < npeaks; i++)
    { /* loop thru the peaks before the largest peak */
    ...
    if (im != -1)
    { /* if there is at least one peak around last pitch */
        if (im == imax)
