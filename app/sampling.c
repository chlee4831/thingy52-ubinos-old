...
static const tes_config_t m_default_config = ENVIRONMENT_CONFIG_DEFAULT
...
static const m_gas_baseline_t m_default_baseline = ENVIRONMENT_BASELINE_DEFAULT
; ///< Default baseline.
...
static uint32_t temperature_start(void)
{
    ...
    err_code = app_timer_start(temperature_timer_id, APP_TIMER_TICKS(m_config.temperature_interval_ms),
    NULL);
    RETURN_IF_ERROR(err_code);
...
static uint32_t pressure_start(void)
{
    ...
    return app_timer_start(pressure_timer_id, APP_TIMER_TICKS(m_config.pressure_interval_ms),
    NULL);
}
...
static uint32_t humidity_start(void)
{
    ...
    return app_timer_start(humidity_timer_id, APP_TIMER_TICKS(m_config.humidity_interval_ms),
    NULL);
}
...
static uint32_t color_start(void)
{
    ...
    return app_timer_start(color_timer_id, APP_TIMER_TICKS(m_config.color_interval_ms),
    NULL);
}
...
static uint32_t config_verify(tes_config_t *p_config)
{
    ...
    if ((p_config->temperature_interval_ms < TES_CONFIG_TEMPERATURE_INT_MIN) || (p_config->temperature_interval_ms > TES_CONFIG_TEMPERATURE_INT_MAX)
            || (p_config->pressure_interval_ms < TES_CONFIG_PRESSURE_INT_MIN) || (p_config->pressure_interval_ms > TES_CONFIG_PRESSURE_INT_MAX)
            || (p_config->humidity_interval_ms < TES_CONFIG_HUMIDITY_INT_MIN) || (p_config->humidity_interval_ms > TES_CONFIG_HUMIDITY_INT_MAX)
            || (p_config->color_interval_ms < TES_CONFIG_COLOR_INT_MIN) || (p_config->color_interval_ms > TES_CONFIG_COLOR_INT_MAX)
            || (p_config->gas_interval_mode < TES_CONFIG_GAS_MODE_MIN) || ((int) p_config->gas_interval_mode > (int) TES_CONFIG_GAS_MODE_MAX))
    {
