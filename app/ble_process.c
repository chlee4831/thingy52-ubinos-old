...
void BLE_process_event_send(uint8_t event, uint8_t status, uint16_t conn_handle, uint16_t handle, uint32_t msg_len, uint8_t *msg)
{
...
{
    return LAP_event_send(LAP_PERIPHERAL_EVT, LAP_PERIPHERAL_ST_CONNECTED, BLE_evt_msg.conn_handle, BLE_evt_msg.handle, BLE_evt_msg.msg_len, NULL);
}

static int processing_BLE_Peripheral_Disconnected(BLEPEvt_msgt BLE_evt_msg)
{
    return LAP_event_send(LAP_PERIPHERAL_EVT, LAP_PERIPHERAL_ST_DISCONNECTED, BLE_evt_msg.conn_handle, BLE_evt_msg.handle, BLE_evt_msg.msg_len, NULL);
}
...
{
    LAP_event_send(LAP_PERIPHERAL_EVT, LAP_PERIPHERAL_ST_CCCD_ENABLED, BLE_evt_msg.conn_handle, BLE_evt_msg.handle, BLE_evt_msg.msg_len, NULL);
}
