...
static uint8_t LAP_adv_data[] =
{
        ...
        0x00,
        0x00,
        0x00,  // anchor node MAC address

        0x00,
        0x00,							// SEND_MSG_UUID_HANDLE(PNIP, APP -> Watch)
        0x00,
        0x00,							// INDI_MSG_UUID_HANDLE(Watch -> APP, PNIP)
        0x00,
        0x00,							// Notification Enable Handle(CCCD)
        };
...
static uint8_t LAP_scan_rsp_data[] =
{
        ...
        0x26,							// profile UUID
        0x02,
        0x7F,							// Rx service UUID
        0x03,
        0x7F,							// Tx service UUID
...
void add_List_LAP_advdata(ble_gap_evt_adv_report_t *pPkt)
{
    ...
    scan_data.profile_data.tx_handle = (pPkt->data.p_data[LAP_ADV_IDX_NOTIFICATION_HANDLE + 1] << 8) | (pPkt->data.p_data[LAP_ADV_IDX_NOTIFICATION_HANDLE]);

    scan_data.profile_data.rx_handle = (pPkt->data.p_data[LAP_ADV_IDX_WRITE_REQUEST_HANDLE + 1] << 8) | (pPkt->data.p_data[LAP_ADV_IDX_WRITE_REQUEST_HANDLE]);

    if (pPkt->data.p_data[LAP_ADV_IDX_EAH1_LENGTH] > 19)
    {
        scan_data.profile_data.cccd_handle = (pPkt->data.p_data[LAP_ADV_IDX_NOTI_EN_HANDLE + 1] << 8) | (pPkt->data.p_data[LAP_ADV_IDX_NOTI_EN_HANDLE]);
    }
